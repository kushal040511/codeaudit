...
python scripts/demo/seed.py --phishing-from-openphish   # ~5–10 min with a local 7B model
...
- **Two contrast scans at pinned commits:** pallets/flask (a healthy library) and excalidraw (a 1,400-file TypeScript graph).
- **Design tokens** for `https://stripe.com`.
- **A phishing analysis** of the newest OpenPhish URL that is still up. Phishing pages die within hours, so no fixed URL can be relied on. The seeded result is what's shown, and `demo/state.json` is gitignored because it holds that live URL.
...
| 2:40 | `state.scans["excalidraw/excalidraw"].url` → Architecture | A 1,400-file graph grouped by directory. Close. |
...
| **Postgres/MinIO state lost, or the wrong machine** | — | `scripts/demo/restore.sh demo/snapshot` (about 30 s), then reload. |
