...
| LLM | Anthropic Python SDK 1.x, `claude-sonnet-4-6` by default (`ANTHROPIC_MODEL`); or a local model through Ollama (`LLM_PROVIDER=ollama`) |
...
Celery worker  run_scan(scan_id)
  ...
  every analyzer failed: retry if infrastructure (back to queued, failed once retries run out), else mark failed · always: remove workspace
```
...
- **Repeats:** within one rule, the i-th heaviest occurrence counts `1/i^0.75`, so pydantic's 14,723 test `assert`s (info, in test code) add a penalty of about 4: less than one error-level finding. See [rubric.md](rubric.md).
...
After analysis the scan is `analysis_complete`: findings, graph and issues are readable. A separate Celery task (`enrich_scan`) then moves it to `enriching` and finally `completed` (or `partial` if analyzers failed). `enrichment_status` (`pending`, `running`, `completed`, `partial`, `failed`, `skipped`) and `enrichment_error` record how the LLM stage went; **nothing in it can fail the scan**. Without a configured LLM (no `ANTHROPIC_API_KEY` with the Anthropic provider, or `LLM_ENABLED=false`), scans go straight to `completed` (or `partial` if an analyzer failed) with enrichment `skipped`.
...
| `GET` | `/ready` | readiness: database, Redis, object storage and a worker heartbeat that reached Docker in the last 90 s. 200 or 503 |
| `GET` | `/metrics` | Prometheus (optional bearer `METRICS_TOKEN`). The worker serves its own on `:9100/metrics` |
| `GET` | `/api/quotas` | your tier, every limit with `used`/`remaining`/`reset_seconds`, monthly LLM tokens, whether suggestions are paused |
| `GET` | `/api/admin/costs?days=30` | operators: LLM spend by day, purpose and user; today vs. the cap |
| `POST` | `/api/admin/llm-kill-switch` | operators: `{"enabled": true, "reason": "…"}` turns every LLM call off immediately |
| `GET` | `/api/admin/dead-letters` | operators: tasks that failed permanently (scrubbed) |
| `PATCH` | `/api/admin/users/{id}/quota` | operators: `{"tier": "pro", "overrides": {"scans": 100}}` |

Every error has the shape `{"error": {"code", "message", "details?"}}`, with codes `invalid_file_type`, `invalid_archive`, `payload_too_large` (413), `not_found` (404), `validation_error` (422), `rate_limited` / `quota_exceeded` (429, with `Retry-After` and a message naming the limit and when it resets) and `service_unavailable` (503). Every API response carries `X-Request-ID` and `X-RateLimit-Limit/Remaining/Reset/Policy`. List endpoints take `page` and `page_size` (max 100) and return `X-Total-Count` and `Link` headers; the bodies stay arrays. Pull requests and API tokens are addressed by UUID. Interactive docs are at http://localhost:8000/docs (disabled when `ENVIRONMENT=production`).
...
```
backend/app/        FastAPI app: api/ (routes), core/, models/, schemas/, services/, workers/
  services/analyzers/   sandbox runner, Semgrep, Bandit, Ruff, OSV-Scanner, dedup, orchestrator
  services/graph/       tree-sitter parser, resolver, layers, metrics, isolated child process
  services/scoring/     rubric, impacts, projection, fix priority
  services/llm/         client (budgets, retries), fix suggester, patch validation, architecture review
  services/github/      OAuth, clone sandbox, PR builder (preview -> confirm)
  services/web/         site analyzer: capture sandbox, egress proxy, risk signals, design tokens
backend/alembic/    migrations (run by the one-shot `migrate` service)
backend/tests/      unit/, integration/, fixtures/ (intentionally vulnerable apps, architecture repos)
frontend/src/       React UI: pages/, components/{scans,architecture,fixes,pullrequests,sites}/
docker/             backend, frontend and web-capture images
action/             GitHub Action (PR score delta)
validation/         rubric/, llm/, phishing/: evaluation harnesses and committed results
scripts/            ci/, loadtest/, demo/ (seed, snapshot/restore, GIF recording)
demo/sample-repo/   the intentionally insecure demo app (also github.com/kushal040511/codeaudit-demo)
docs/               architecture, ADRs, rubric, validation, security, demo script, this reference
```
...
Migrations run in the one-shot `migrate` service before the API starts; the API itself never migrates. The API hot reloads `backend/app`. **The worker and beat do not hot reload:** run `docker compose restart worker beat` after changing backend code.
...
5. **The git clone container has general network egress.** Restrict it on the worker VM (see Deploying).

The full sandbox audit, CodeAudit's results on its own code and every issue found and fixed are in [docs/security-review.md](security-review.md).

## Production operations

**Quotas.**
- Rolling windows in Redis (atomic Lua sorted sets), per user or per client IP when signed out, with tiers `anonymous` / `free` / `pro`. Per-user overrides (`users.quota_*`) are set via the admin API.
- Scans, site analyses, pull requests and PR previews per day or hour, plus API requests per minute. Checked at the API. The LLM limits (daily spend cap, kill switch, monthly tokens) are checked again where enrichment is dispatched and before each call.
- Monthly LLM tokens per user, with anonymous users sharing one pool.
- `X-Forwarded-For` is honoured only for `TRUSTED_PROXY_HOPS`. The UI shows what's left on the scan page and in Settings.

**Cost controls.**
- **Per-scan token ceiling:** every LLM call reserves its worst case against `LLM_TOKEN_BUDGET_PER_SCAN` before it is sent.
- **Daily spend cap:** `LLM_DAILY_SPEND_CAP_USD` counts finished calls plus in-flight estimates, under a Postgres advisory lock so concurrent workers can't overshoot. It is checked before enrichment is dispatched and before each call.
- **When suggestions stop:** a scan still completes. Enrichment ends `partial` (if some suggestions were produced) or `skipped`, with the reason.
- **Kill switch and alerts:** the kill switch is one Redis key. Alerts fire once per day at `LLM_SPEND_ALERT_RATIO` and at the cap, as a warning log plus an optional `ALERT_WEBHOOK_URL`.

**Observability.**
- **Logs:** JSON, with `request_id` carried from the API into Celery task headers and bound in the worker (`task_id`, `scan_id`/`site_analysis_id`). Secrets are scrubbed from every log line, dead letter and Sentry event, and user code never appears in logs.
- **Metrics:** HTTP by route and status, scan stage durations, analyzer runs, sandbox slots, wait time and reaps, LLM latency, tokens and cost, patch validation results, quota rejections, queue depth, jobs by status, spend vs. cap.
- **Health endpoints:** `/health` is liveness, `/ready` is readiness.

**Resilience.**
- **Duplicate and failed deliveries:** tasks are acknowledged late and requeued if the worker dies. A scan is claimed atomically, so duplicate deliveries are no-ops. Findings are inserted idempotently (`UNIQUE (scan_id, fingerprint)`). Permanently failed tasks are recorded as dead letters, and the scan row shows `failed` with the reason.
- **Beat reapers (every 5 min):** the stale-job reaper fails scans, site analyses and PR creations stuck past their limits, and closes stuck enrichments as `failed` (the scan itself stays `completed` or `partial`). The sandbox reaper removes containers from dead workers.
- **Shutdown:** warm (SIGTERM, `stop_grace_period: 10m` on the worker).

**Caching.**
- **Scans:** an identical upload (SHA-256 of the bytes) or the same repository commit, analyzed by the same analyzer images and rubric within `SCAN_CACHE_TTL_SECONDS` (7 days), returns the earlier scan (`"cached": true`). Only anonymous scans or your own are reused.
- **Site analyses:** reused for `SITE_CACHE_TTL_SECONDS` (6 h).

**Performance.** Query plans for the hot paths on a 2M-finding dataset are in [docs/query-performance.md](query-performance.md). **Load:** 20 concurrent mid-size scans complete in 44 s (p95 44 s end to end) with no failures (re-run 2026-09-18). The first bottleneck is CPU (Semgrep); see [docs/load-test.md](load-test.md).

## Deploying

**Decision: API on a PaaS, analysis workers on a VM with Docker.** Render, Railway and similar PaaS give containers no Docker daemon and don't allow Docker-in-Docker, and the analyzers must run in sibling sandbox containers. So the web-facing part and the worker are deployed separately:

| Part | Where | Needs |
|---|---|---|
| `web` (Caddy + SPA), `api` | Render / Railway / Fly.io apps (no Docker socket needed) | `docker/frontend/Dockerfile`, `docker/backend/Dockerfile` |
| Postgres, Redis, S3 | Managed services (Render Postgres, Upstash / Render Redis, S3 / R2) | Private networking or TLS URLs |
| `worker`, `beat`, `egress-proxy` | One or more Linux VMs with Docker: Fly Machines on a dedicated host, EC2, DigitalOcean. 8 vCPU / 16 GB serves about 20 scans/min. | `docker-compose.prod.yml` (worker services only), `/var/run/docker.sock` |

**1. Images.** CI builds both images (`.github/workflows/ci.yml`). Push them to a registry and set `BACKEND_IMAGE` / `WEB_IMAGE`. Both are multi-stage, non-root and contain no compilers.

**2. Configuration.** Start from `.env.example`; a unit test fails if it misses a setting. In production set:
- `ENVIRONMENT=production`, `SESSION_COOKIE_SECURE=true` and `TOKEN_ENCRYPTION_KEYS`
- `TRUSTED_PROXY_HOPS` (1 behind Caddy; add one for the PaaS router)
- `METRICS_TOKEN`, `SENTRY_DSN`, `LLM_DAILY_SPEND_CAP_USD`, `ALERT_WEBHOOK_URL`
- `CORS_ORIGINS` and `FRONTEND_URL`
- a Redis URL with its password, e.g. `redis://:PASSWORD@host:6379/0`

**3. Migrations (release step, never on boot).** Run `alembic upgrade head` once per deploy before new API instances start:
- **Render:** Pre-Deploy Command.
- **Railway:** `railway run alembic upgrade head` in the deploy pipeline.
- **Fly.io:** `[deploy] release_command = "alembic upgrade head"`.
- **VM:** `docker compose -f docker-compose.prod.yml --profile migrate run --rm migrate`.

Migrations are written to be backwards compatible with the previous release: add columns, backfill, then constrain.

**4. API on the PaaS.** Command: `uvicorn app.main:app --host 0.0.0.0 --port $PORT --proxy-headers --forwarded-allow-ips '*'`. Health check path: `/health`; use `/ready` for alerting, not restarts. Run one process per instance and scale instances, not uvicorn workers.

**5. Worker VM.**
```bash
# on the VM (Docker installed); .env.production holds the same settings as the API
docker compose -f docker-compose.prod.yml --env-file .env.production up -d worker beat egress-proxy
docker build -t codeaudit-web-capture:1.63.0 docker/web-capture   # page-capture sandbox image
```
Hardening for the VM:
- **Dedicated host.** Nothing else runs on it. Use a security group with no inbound ports except SSH and `9100` from your Prometheus.
- **Block private and metadata addresses from sandbox containers.** The analyzers already have no network; this covers the git clone:
  ```bash
  iptables -I DOCKER-USER -d 169.254.169.254 -j DROP          # cloud metadata
  iptables -I DOCKER-USER -s 172.17.0.0/16 -d 10.0.0.0/8 -j DROP   # default bridge -> your VPC
  ```
  Use your VPC's CIDR and the bridge subnet shown by `docker network inspect bridge`. Don't blanket-drop `172.16.0.0/12`: the compose networks (worker ↔ egress proxy) live there. The worker's own traffic to managed Postgres, Redis and S3 is unaffected.
- **Stronger isolation (recommended):** install gVisor and make it the default runtime (`"default-runtime": "runsc"` in `/etc/docker/daemon.json`). Every sandbox image works under it. Semgrep runs about 20–30% slower.
- **Sizing:**
  - `MAX_CONCURRENT_SANDBOXES` ≈ vCPUs ÷ 2 per VM. The semaphore is global, so add the same amount for each VM you add.
  - `CELERY_CONCURRENCY` ≈ slots + 2.
- **Graceful deploys:** `docker compose stop` sends SIGTERM, and the worker finishes running scans within `stop_grace_period` (10 min). Anything cut off is redelivered and resumed, or failed by the reaper with a message.
- **Exactly one `beat`** in the whole deployment.

**Docker-in-Docker caveat.** Don't run the worker inside a PaaS container with a nested Docker daemon (`docker:dind`). It needs `--privileged`, which removes the isolation the sandbox relies on and gives untrusted code a path to the host. Alternatives when no VM is available:
- **Firejail or bubblewrap** inside the worker container (namespaces plus seccomp, no daemon), at the cost of the per-image toolchain isolation.
- **gVisor's `runsc do`** to run tools in-process.
- **A job runner with a real sandbox API** (Fly Machines API, AWS Fargate / ECS RunTask, Kubernetes Jobs with gVisor) replacing `run_in_sandbox`.

**All-in-one (single VM).** `docker-compose.prod.yml` runs the whole stack on one host, with resource limits, read-only root filesystems and `cap_drop: ALL` on every service:
```bash
...
```
Put TLS in front: a PaaS edge, a load balancer, or Caddy with a hostname instead of `:8080`, which gets certificates automatically.

## Continuous integration

`.github/workflows/ci.yml` runs on every push and pull request:
- **Lint:** ruff, black and mypy.
- **Unit tests.**
- **Integration tests:** Postgres, Redis and MinIO services with real sandboxed analyzers; migrations `upgrade → check → downgrade → upgrade`.
- **Frontend:** lint, typecheck and build.
- **Dependency audit:** OSV-Scanner over `uv.lock` and `package-lock.json`. `scripts/ci/dependency_gate.py` **fails on any critical (CVSS ≥ 9 or CRITICAL) vulnerability** not in `.github/dependency-allowlist.txt` with an unexpired date. pip-audit and `npm audit --audit-level=critical` also run.
- **Docker builds:** both images, a check that they run as non-root, and compose validation.
