...
The architecture analyzer adds a harder case. tree-sitter is native code parsing untrusted input. The README records that py-tree-sitter 0.26.0 corrupted memory (SIGBUS/SIGSEGV) after parsing a few files in one process. It is pinned below 0.26 (`backend/pyproject.toml`). With `task_acks_late` and `task_reject_on_worker_lost` (`backend/app/core/celery_app.py`), an in-process crash would kill the worker, and the redelivered scan would crash the next worker too.
...
- **The score is flagged.** `rubric.score` excludes a category whose analyzer failed and adds an `incomplete_reasons` entry. The score is marked `incomplete`, and the README says it shouldn't be compared with complete scores. A category that doesn't apply (for example, no Python) is excluded without the flag.
...
- One flaky tool degrades a scan to `partial` instead of losing it. The UI shows which tools failed (README, "Failure isolation").
...
- Consumers must handle `partial` and `incomplete` as well as `completed`. The frontend has a "Partial results" banner (README, "Frontend").
- The child process adds interpreter start-up time. The README reports the pydantic architecture run at 2.1 s through the whole stack, including process start-up, against 1.04 s measured directly.
- The child process has no network or resource limits beyond a timeout (README, "Known gaps" item 4).
...
- **Run tree-sitter in a sandbox container.** A stronger boundary, and listed as a future improvement in README, "Known gaps" item 4. It isn't done because it needs another image and the container overhead.
