...
The full sandbox audit, CodeAudit's results on its own code and every issue found and fixed are in [docs/security-review.md](docs/security-review.md).
...
- Scans, site analyses, pull requests and PR previews per day or hour, plus API requests per minute. Checked at the API and again where work is dispatched.
...
- **Beat reapers (every 5 min):** the stale-job reaper fails scans, enrichments, site analyses and PR creations stuck past their limits. The sandbox reaper removes containers from dead workers.
...
**Performance.** Query plans for the hot paths on a 2M-finding dataset are in [docs/query-performance.md](docs/query-performance.md). **Load:** 20 concurrent mid-size scans complete in 51 s with no failures. The first bottleneck is CPU (Semgrep); see [docs/load-test.md](docs/load-test.md).
